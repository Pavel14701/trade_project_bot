...
class Schema:
    ...
    Attributes:
        ...
        cat_vocabs (Dict[str, Dict[str, int]]): Mapping from category values 
          to integer indices per column.
        cat_unk_idx (Dict[str, int]): Reserved index for unknown 
          categorical values per column.
    """
    num_cols: List[str]
    cat_cols: List[str]
...
class SchemaInfer:
    ...
    Parameters:
        ...
        min_freq (int): Minimum frequency required for a categorical 
    ...
    def fit(
        ...
        cat_cols = [
            ...
            if col not in exclude and col not in num_cols  # type: ignore
        ]
        cat_vocabs: Dict[str, Dict[str, int]] = {}
        cat_unk_idx: Dict[str, int] = {}
        for col in cat_cols:
    ...
    def __repr__(self) -> str:
        ...
        return f"SchemaInfer(fit=True, num={len(self._schema.num_cols)}, \
            cat={len(self._schema.cat_cols)})"
