"""
Provides a factory function for creating a configured instance of `RabbitBroker` using 
  application-level settings.

This module integrates FastStream's RabbitMQ broker with SASL/PLAIN 
  authentication, based on values
defined in the `RabbitMQConfig` dataclass. It is typically used to 
...
def new_broker(rabbitmq_config: RabbitMQConfig) -> RabbitBroker:
    ...
    This function sets up the broker with host, port, virtual host, 
      and SASL/PLAIN authentication
based on the provided configuration object. It is intended to be used during 
  application startup
to initialize message routing.

    Args:
        rabbitmq_config (RabbitMQConfig): Configuration object containing RabbitMQ 
          connection details.
