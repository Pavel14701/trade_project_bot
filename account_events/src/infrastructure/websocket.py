...
class OKXWebsocketsChannelGateway:
    def __init__(self, broker: RabbitBroker) -> None:
        """Инициализация WebSocket с возможностью работы сразу с несколькими пользователями."""
    ...
    async def connect(self, config: WebSocketDM) -> None:
        """Открывает WebSocket-соединение для конкретного пользователя и управляет всеми активными подключениями."""
    ...
    async def subscribe(self, config: WebSocketDM) -> None:
        ...
        subscriptions: list[dict] = [
            {"channel": event, "instType": config.instType} if event in ["positions", "liquidation-warning"] 
            else {"channel": event}
            for event in ["account", "positions", "liquidation-warning"] if getattr(config, event)
        ]
        if subscriptions:
    ...
    async def message_handler(self, user_id: int, message: str) -> None:
        ...
        await self._broker.publish(
            RabbitMessage({"user_id": user_id, "message": message}),
